// Footer — brand mark, navigation, social links, and copyright.
// Static content, so this stays a server component (no "use client").
import Link from "next/link";
import { siteConfig, navLinks, socialLinks, legalLinks } from "@/lib/site";
import { Container } from "@/components/ui/Container";

// Inline SVG paths for each social platform, keyed by the `icon` field in
// socialLinks. Inline SVGs avoid pulling in an icon library dependency.
  ...
  return (
    <footer className="relative mt-20 border-t border-white/10">
      <Container size="wide" className="py-14">
                ...
                {socialLinks.map((link) => (
                  <li key={link.label}>
                    <a
                      ...
                      title={link.label}
                      className="flex h-9 w-9 items-center justify-center rounded-full border border-white/10 text-muted transition-colors hover:border-gold-400/50 hover:bg-white/5 hover:text-gold-400 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-gold-400/60"
                    >
                      <svg
                        viewBox="0 0 24 24"
                        fill="currentColor"
        ...
        {/* Legal links */}
        <nav className="mt-12 flex flex-wrap gap-x-6 gap-y-2 border-t border-white/10 pt-6">
          {legalLinks.map((link) => (
            <Link
              key={link.href}
              href={link.href}
